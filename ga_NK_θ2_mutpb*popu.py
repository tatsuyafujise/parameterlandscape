...
random.seed(14) # NK参照表のスコアのための乱数
...
def main():
    ...
    population_size = 500
    csv_filename = "ga_results_NK_mutpb*populationsize.csv"
    data = []
    heatdict = {}
    ...
    # FDCの計算
    dist = [np.linalg.norm([(data_sorted[i][0] - data_best_θ1) / 998, (data_sorted[i][1] - data_best_θ2) / 0.099], ord=2) for i in range(1000)]
    print(dist)
    ...
    ax1.set_title('NK_fdc_population size vs mutpb')
    ax1.grid(True)
